...
def lambda_handler(event, context):
  try:
    ...
    for meal in three_meals:
      ...
      #
      custo_sql = "SELECT foodid, type FROM user_customize WHERE userid = %s"
      custo_row = datatier.retrieve_all_rows(dbConn, custo_sql, [userid])

      allergy = [food[0] for food in custo_row if food[1] == "allergy"]
      like = [food[0] for food in custo_row if food[1] == "like"]
      dislike = [food[0] for food in custo_row if food[1] == "dislike"]
      #
      ...
      for typ in food_type:
        ...
        for food in food_cat:
          sql = """
            SELECT f.foodid, f.foodname, f.calories
            FROM food f
            JOIN food_type ft ON f.foodid = ft.foodid
            WHERE ft.Category = %s 
          """
          result = datatier.retrieve_all_rows(dbConn, sql, [food])
          result = [row for row in result if row[0] not in allergy]
          ...
          for row in result:
              ...
              if foodid in like:
                  weights.append(3) 
              elif foodid in dislike:
                  weights.append(0.5)  
              else:
                  weights.append(1)  
          if result:
              selected = random.choices(result, weights=weights, k=1)[0]
              food_with_cal.append(selected)
          else:
              print("No suitable food found for category:", food)
        ...
        food_with_gram = [[fc[1], round(per_food_cal/fc[2], 1)] for fc in food_with_cal]
